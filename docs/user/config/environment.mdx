---
title: "Environment"
description: "Environment variables and settings"
---

## Environment Variables

ichat uses environment variables for configuration. No config files needed!

### Required

| Variable | Description | Example |
|----------|-------------|---------|
| `API_KEY` | OpenRouter or OpenAI API key | `sk-or-v1-...` |

### Optional

| Variable | Description | Default |
|----------|-------------|---------|
| `API_BASE` | Custom API endpoint | `https://openrouter.ai/api` |
| `DATA_PATH` | Storage directory | `.` (current dir) |
| `BIND_ADDR` | Network socket | `0.0.0.0:80` |
| `TRUSTED_HEADER` | middleware auth | None |
| `NEWAPI_AUTH_BASE` | Base URL for New API user auth | None |
| `NEWAPI_AUTH_BEARER` | Optional service token for upstream auth calls | None |
| `NEWAPI_AUTH_USER_HEADER` | Upstream user header name when needed | `New-Api-User` |

## Docker Configuration

### Example

```bash
docker run -d \
  -e API_KEY="sk-or-v1-..." \
  -e DATA_PATH="/data" \
  -e BIND_ADDR="0.0.0.0:8080" \
  -p 8080:8080 \
  -v ./ichat-data:/data \
  ghcr.io/isomoes/ichat-web:latest
```

## API Provider Configuration

### OpenRouter (Recommended)

1. Sign up at [openrouter.ai](https://openrouter.ai)
2. Generate an API key
3. Set the key:

```bash
export API_KEY="sk-or-v1-..."
# API_BASE defaults to OpenRouter, no need to set
```

**Why OpenRouter?**
- Access to 100+ models
- Pay-per-use pricing
- No subscriptions
- Automatic fallbacks

### OpenAI

```bash
export API_KEY="sk-..."
export API_BASE="https://api.openai.com"
```

### Local Models (Ollama)

```bash
# Start Ollama
ollama serve

# Configure ichat
export API_KEY="ollama"
export API_BASE="http://localhost:11434/v1"
```

### Custom Endpoints

Any OpenAI-compatible API works:

```bash
export API_KEY="your-key"
export API_BASE="https://your-endpoint.com/v1"
```

### External User Auth With New API

To delegate login and registration to a separate New API deployment while keeping ichat conversation history local:

```bash
export NEWAPI_AUTH_BASE="https://newapi.example.com"
```

Optional upstream settings:

```bash
export NEWAPI_AUTH_BEARER="optional-service-token"
export NEWAPI_AUTH_USER_HEADER="New-Api-User"
```

Behavior:

- Your browser still talks only to ichat
- ichat backend calls New API server-to-server
- Conversation history, preferences, and file ownership stay in ichat's local database
- Different users cannot see each other's local history because ichat still scopes chats by local owner id

## Storage Configuration

### Data Directory

ichat stores:
- Conversation history
- User preferences
- Session data
- Uploaded files (temporary)

**Docker:**
```bash
-v ./ichat-data:/data
-e DATA_PATH=/data
```

**Native:**
```bash
export DATA_PATH=/var/lib/ichat
mkdir -p $DATA_PATH
./ichat
```

### Data Structure

```
data/
├── conversations/
│   └── *.json
├── users/
│   └── users.db
└── uploads/
    └── (temporary files)
```

## Network Configuration

### Binding Address

**Listen on all interfaces (default):**
```bash
BIND_ADDR=0.0.0.0:80
```

**Localhost only:**
```bash
BIND_ADDR=127.0.0.1:80
```

**Custom port:**
```bash
BIND_ADDR=0.0.0.0:8080
```

### Behind Reverse Proxy

If using Nginx, Caddy, or similar:

```bash
# ichat listens locally
BIND_ADDR=127.0.0.1:3000

# Reverse proxy handles external access
```

Example Nginx config:

```nginx
server {
    listen 80;
    server_name ichat.example.com;
    
    location / {
        proxy_pass http://127.0.0.1:3000;
        proxy_http_version 1.1;
        proxy_set_header Upgrade $http_upgrade;
        proxy_set_header Connection 'upgrade';
        proxy_set_header Host $host;
        proxy_cache_bypass $http_upgrade;
    }
}
```

## UI Settings

Settings configurable through the web interface:

### Appearance
- Theme selection
- Dark/light mode
- Font size
- Pattern overlays

### Chat Behavior
- Default chat mode
- Token streaming
- Message formatting
- Auto-scroll

### Privacy
- Conversation history
- Anonymous usage
- Local storage only

## Advanced Configuration

### Multiple Instances

Run multiple ichat instances:

```bash
# Instance 1
docker run -d --name ichat-1 -p 8001:80 -e API_KEY="..." -v ./data1:/data ghcr.io/isomoes/ichat-web:latest

# Instance 2
docker run -d --name ichat-2 -p 8002:80 -e API_KEY="..." -v ./data2:/data ghcr.io/isomoes/ichat-web:latest
```

### Custom API Models

Configure specific models in the UI after login.

### Resource Limits

Docker resource constraints:

```bash
docker run -d \
  --memory="256m" \
  --cpus="1.0" \
  -e API_KEY="..." \
  ghcr.io/isomoes/ichat-web:latest
```

## Security Best Practices

<Accordion title="External account management">
  - If `NEWAPI_AUTH_BASE` is configured, user login and registration are delegated to your New API instance.
  - ichat stores only local conversation history, preferences, and file ownership.
  - Password resets and account recovery happen in your New API deployment.
</Accordion>

<Accordion title="Protect API keys">
  - Never commit `.env` files to git
  - Use secrets management in production
  - Rotate keys periodically
  - Use separate keys per instance
</Accordion>

<Accordion title="Network security">
  - Use HTTPS in production (reverse proxy)
  - Restrict access with firewall rules
  - Consider VPN for remote access
  - Don't expose to public internet without auth
</Accordion>
