...
To delegate login and registration to a separate New API deployment while keeping llumen conversation history local:
...
- Your browser still talks only to llumen
- llumen backend calls New API server-to-server
- Conversation history, preferences, and file ownership stay in llumen's local database
- Different users cannot see each other's local history because llumen still scopes chats by local owner id
